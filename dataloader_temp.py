...
class UCF_VideoDataset:
    def __init__(self, folder_path, readfile):
        ...
        # self.label2index = {label: index for index, label in enumerate(sorted(set(folder_list)))}
        # self.label_array = np.array([self.label2index[label] for label in folder_list], dtype=int)
    ...
    def __getitem__(self, idx):
        ...
        time_index = np.random.randint(buffer.shape[1] - self.clip_len)
